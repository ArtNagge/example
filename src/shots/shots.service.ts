...
import { COSTS, ShotsStatusEnum } from './types/enums/shots';
...
export class ShotsService {
  ...
  findAndCount = async (filter: ShotsFilterDto, userId?: string) =>
    this.shotsRepository.findAllAndCount(filter, userId);

  findById = async (id: string) => this.shotsRepository.findOneById(id);
  ...
  create = async (shot: ShotsCreateDtoProps) => {
    ...
    const priceTask = COSTS[findType.slug] * count;
  ...
  remove = async (userId: string, shotId: string) => {
    ...
    const { count, executions } = shot;

    const returnPrice = (count - executions) * COSTS[shot.type.slug];
  ...
  perform = async (userId: string, shotId: string) => {
    ...
    await this.usersService.change(userId, {
      balance: user.balance + COSTS[typeSlug],
    });
